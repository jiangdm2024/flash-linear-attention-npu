...
int main() {
  ...
  ret = aclnnChunkBwdDvLocalGetWorkspaceSize(q, k, dO, g, cuSeqlens, chunkIndices, scale, chunk_size, dV, &workspaceSize, &executor);
  CHECK_RET(
      ret == ACL_SUCCESS,
      LOG_PRINT("aclnnChunkBwdDvLocalGetWorkspaceSize failed. ERROR: %d\n", ret);
      return ret);
  ...
  CHECK_RET(ret == ACL_SUCCESS,
            LOG_PRINT("aclnnChunkBwdDvLocal failed. ERROR: %d\n", ret);
            return ret);
